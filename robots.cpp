...
void Arena::display() const
{
	...
	for (int k = 0; k < m_nRobots; k++)
		if (nRobotsAt(m_robots[k]->row(), m_robots[k]->col()) == 1)
			grid[m_robots[k]->row() - 1][m_robots[k]->col() - 1] = 'R';
		else if (nRobotsAt(m_robots[k]->row(), m_robots[k]->col()) < 9)
			grid[m_robots[k]->row() - 1][m_robots[k]->col() - 1] = nRobotsAt(m_robots[k]->row(), m_robots[k]->col()) + 48; 
		else 
